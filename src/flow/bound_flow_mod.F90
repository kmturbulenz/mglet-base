...
CONTAINS
    ...
    SUBROUTINE apply_bound_flow_impl(ilevel_index, ntasks, u, v, w, p, &
        ...
        DO itask = 1, ntasks
            ...
            IF (ityp == 3 .OR. ityp == 4) THEN
                !$omp parallel
                CALL bound_nobuffer_device(kk, jj, ii, iface, ityp, &
    ...
    SUBROUTINE bound_nobuffer_device(kk, jj, ii, iface, ityp, u, v, w, p)
        !$omp declare target
    ...
    SUBROUTINE bfront_velocity_device(kk, jj, ii, i2, i3, i4, istag1, &
        ...
        CASE (1)
            ...
            !$omp end do
        CASE (2)
            !$omp do collapse(2) private(j, k, flag)
            DO j = 1, jj-1
                DO k = 1, kk-1
        ...
        CASE (4)
            ...
            !$omp end do
        CASE (5)
            !$omp do collapse(2) private(j, k, sbv, sbw)
            DO j = 2, jj-1
                DO k = 2, kk-1
            ...
            !$omp do collapse(2) private(j, k, sbu, sb11, sb12, &
            !$omp& sb13, sb14, fak, ubfine)
            DO j = 3, jj-2, 2
                DO k = 3, kk-2, 2
                    sbu = bp(k, j, i2)
                    IF (sbu > 0.5) THEN
                    ...
                    ELSE
                        sb11 = MIN(bp(k, j, i3) + bp(k, j, i2) &
                            + bp(k, j, i4) + bp(k, j-1, i3) &
                            + bp(k, j+1, i3) + bp(k-1, j, i3) &
                            + bp(k+1, j, i3), 1.0_realk)
                        sb12 = MIN(bp(k, j+1, i3) + bp(k, j+1, i2) &
                            + bp(k, j+1, i4) + bp(k, j, i3) &
                            + bp(k, j+2, i3) + bp(k-1, j+1, i3) &
                            + bp(k+1, j+1, i3), 1.0_realk)
                        sb13 = MIN(bp(k+1, j, i3) + bp(k+1, j, i2) &
                            + bp(k+1, j, i4) + bp(k+1, j-1, i3) &
                            + bp(k+1, j+1, i3) + bp(k, j, i3) &
                            + bp(k+2, j, i3), 1.0_realk)
                        sb14 = MIN(bp(k+1, j+1, i3) &
                            + bp(k+1, j+1, i2) + bp(k+1, j+1, i4) &
                            + bp(k+1, j, i3) + bp(k+1, j+2, i3) &
                            + bp(k, j+1, i3) + bp(k+2, j+1, i3), &
                            1.0_realk)
                    END IF
                    fak = 1.0/((ddy(j)+ddy(j+1))*(ddz(k)+ddz(k+1))) &
                        * (sb11*ddy(j)*ddz(k) + sb12*ddy(j+1)*ddz(k) &
                        + sb13*ddy(j)*ddz(k+1) &
        ...
        IF (ityp == 2) THEN
            ...
            DO j = 2, jj
                DO k = 2, kk
                    ...
                    p(k, j, i2) = pinf + MIN(0.0_realk, &
                        0.5*rho*SIGN(umagsqr, dir*u(k, j, istag2)))
                END DO
            END DO
            !$omp end do
        ELSE IF (ityp == 1 .OR. ityp == 3 .OR. ityp == 4) THEN
            !$omp do collapse(2) private(j, k)
            DO j = 1, jj
                DO k = 1, kk
                    p(k, j, i2) = p(k, j, i3)
    ...
    SUBROUTINE bright_velocity_device(kk, jj, ii, j2, j3, j4, jstag1, &
        ...
        CASE (1)
            ...
            !$omp end do
        CASE (2)
            !$omp do collapse(2) private(i, k, flag)
            DO i = 1, ii-1
                DO k = 1, kk-1
                    flag = SIGN(1.0_realk, &
        ...
        CASE (4)
            ...
            !$omp end do
        CASE (5)
            !$omp do collapse(2) private(i, k, sbu, sbw)
            DO i = 2, ii-1
                DO k = 2, kk-1
            ...
            !$omp do collapse(2) private(i, k, sbv, sb11, sb12, &
            !$omp& sb13, sb14, fak, vbfine)
            DO i = 3, ii-2, 2
                DO k = 3, kk-2, 2
                    sbv = bp(k, j2, i)
                    IF (sbv > 0.5) THEN
                    ...
                    ELSE
                        sb11 = MIN(bp(k, j3, i) + bp(k, j2, i) &
                            + bp(k, j4, i) + bp(k, j3, i-1) &
                            + bp(k, j3, i+1) + bp(k-1, j3, i) &
                            + bp(k+1, j3, i), 1.0_realk)
                        sb12 = MIN(bp(k, j3, i+1) + bp(k, j2, i+1) &
                            + bp(k, j4, i+1) + bp(k, j3, i) &
                            + bp(k, j3, i+2) + bp(k-1, j3, i+1) &
                            + bp(k+1, j3, i+1), 1.0_realk)
                        sb13 = MIN(bp(k+1, j3, i) + bp(k+1, j2, i) &
                            + bp(k+1, j4, i) + bp(k+1, j3, i-1) &
                            + bp(k+1, j3, i+1) + bp(k, j3, i) &
                            + bp(k+2, j3, i), 1.0_realk)
                        sb14 = MIN(bp(k+1, j3, i+1) &
                            + bp(k+1, j2, i+1) + bp(k+1, j4, i+1) &
                            + bp(k+1, j3, i) + bp(k+1, j3, i+2) &
                            + bp(k, j3, i+1) + bp(k+2, j3, i+1), &
                            1.0_realk)
                    END IF
                    fak = 1.0/((ddz(k)+ddz(k+1))*(ddx(i)+ddx(i+1))) &
                        * (sb11*ddz(k)*ddx(i) + sb12*ddz(k)*ddx(i+1) &
                        + sb13*ddz(k+1)*ddx(i) &
                    ...
                    vbfine = (v(k, jstag2, i)*(1.0-sb11)*ddz(k)*ddx(i) &
                        ...
                        + v(k+1, jstag2, i+1)*(1.0-sb14) &
                        * ddz(k+1)*ddx(i+1)) &
                        /((ddz(k)+ddz(k+1))*(ddx(i)+ddx(i+1)))
                    v(k, jstag2, i) = (vbuf(k, i)-vbfine)*fak*sb11 &
                        + v(k, jstag2, i)*(1.0-sb11)*sbv
                    v(k, jstag2, i+1) = (vbuf(k, i+1)-vbfine)*fak*sb12 &
                        + v(k, jstag2, i+1)*(1.0-sb12)*sbv
                    v(k+1, jstag2, i) = &
                        (vbuf(k+1, i)-vbfine)*fak*sb13 &
                        + v(k+1, jstag2, i)*(1.0-sb13)*sbv
                    v(k+1, jstag2, i+1) = &
                        (vbuf(k+1, i+1)-vbfine)*fak*sb14 &
                        + v(k+1, jstag2, i+1)*(1.0-sb14)*sbv
                END DO
            END DO
            !$omp end do
        END SELECT
    ...
    SUBROUTINE bbottom_velocity_device(kk, jj, ii, k2, k3, k4, kstag1, &
        ...
        CASE (1)
            ...
            !$omp end do
        CASE (2)
            !$omp do collapse(2) private(i, j, flag)
            DO i = 1, ii-1
                DO j = 1, jj-1
                    flag = SIGN(1.0_realk, &
                    ...
                    u(k2, j, i) = flag*u(k3, j, i) &
                        + (1.0-flag)*(2.0*ubuf(j, i) - u(k3, j, i))
                    flag = SIGN(1.0_realk, &
        ...
        CASE (4)
            ...
            !$omp end do
        CASE (5)
            !$omp do collapse(2) private(i, j, sbu, sbv)
            DO i = 2, ii-1
                DO j = 2, jj-1
            ...
            !$omp do collapse(2) private(i, j, sbw, sb11, sb12, &
            !$omp& sb13, sb14, fak, wbfine)
            DO i = 3, ii-2, 2
                DO j = 3, jj-2, 2
                    sbw = bp(k2, j, i)
                    IF (sbw > 0.5) THEN
                    ...
                    ELSE
                        sb11 = MIN(bp(k3, j, i) + bp(k2, j, i) &
                            + bp(k4, j, i) + bp(k3, j, i-1) &
                            + bp(k3, j, i+1) + bp(k3, j-1, i) &
                            + bp(k3, j+1, i), 1.0_realk)
                        sb12 = MIN(bp(k3, j, i+1) + bp(k2, j, i+1) &
                            + bp(k4, j, i+1) + bp(k3, j, i) &
                            + bp(k3, j, i+2) + bp(k3, j-1, i+1) &
                            + bp(k3, j+1, i+1), 1.0_realk)
                        sb13 = MIN(bp(k3, j+1, i) + bp(k2, j+1, i) &
                            + bp(k4, j+1, i) + bp(k3, j+1, i-1) &
                            + bp(k3, j+1, i+1) + bp(k3, j, i) &
                            + bp(k3, j+2, i), 1.0_realk)
                        sb14 = MIN(bp(k3, j+1, i+1) &
                            + bp(k2, j+1, i+1) + bp(k4, j+1, i+1) &
                            + bp(k3, j+1, i) + bp(k3, j+1, i+2) &
                            + bp(k3, j, i+1) + bp(k3, j+2, i+1), &
                            1.0_realk)
                    END IF
                    fak = 1.0/((ddy(j)+ddy(j+1))*(ddx(i)+ddx(i+1))) &
                        * (sb11*ddy(j)*ddx(i) + sb12*ddy(j)*ddx(i+1) &
                        + sb13*ddy(j+1)*ddx(i) &
                    ...
                    w(kstag2, j+1, i) = &
                        (wbuf(j+1, i)-wbfine)*fak*sb13 &
                        + w(kstag2, j+1, i)*(1.0-sb13)*sbw
                    w(kstag2, j+1, i+1) = &
                        (wbuf(j+1, i+1)-wbfine)*fak*sb14 &
