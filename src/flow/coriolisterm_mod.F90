...
CONTAINS
    ...
    SUBROUTINE init_coriolisterm()
        ...
        IF ( fort7%exists("/flow/coriolis") ) THEN
        ...
        IF ( has_coriolis ) THEN
            ...
            IF ( ABS(omega(1)) + ABS(omega(3)) > 0.0 ) THEN
                DO i = 3, ii-2
                    DO j = 3-nrv, jj-3+nlv
                        DO k = 3, kk-2
                            ! averaging to approximate V-velocity point (stag=0,1,0)
                            vlocal(1) = 0.25 * (u(k,j,i-1) + u(k,j+1,i-1) + u(k,j,i) + u(k,j+1,i))
                            vlocal(2) = v(k,j,i)
                            vlocal(3) = 0.25 * (w(k-1,j,i) + w(k-1,j+1,i) + w(k,j,i) + w(k,j+1,i))
                            ! computing the cross product 
                            cterm = omega(3) * vlocal(1) - omega(1) * vlocal(3)
                            ! adding to the momentum balance
            ...
            IF ( ABS(omega(1)) + ABS(omega(2)) > 0.0 ) THEN
                DO i = 3, ii-2
                    DO j = 3, jj-2
                        DO k = 3-nbw, kk-3+ntw
                            ! averaging to approximate W-velocity point (stag=0,0,1)
                            vlocal(1) = 0.25 * (u(k,j,i-1) + u(k+1,j,i-1) + u(k,j,i) + u(k+1,j,i))
                            vlocal(2) = 0.25 * (v(k,j-1,i) + v(k+1,j-1,i) + v(k,j,i) + v(k+1,j,i)) 
                            vlocal(3) = w(k,j,i)
                            ! computing the cross product 
                            cterm = omega(1) * vlocal(2) - omega(2) * vlocal(1)
                            ! adding to the momentum balance
