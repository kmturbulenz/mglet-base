MODULE bound_pressure_mod
    ...
    ABSTRACT INTERFACE
        SUBROUTINE boundary_callback_t(ilevel_index, igrid, iface, iworkpackage)
            IMPORT :: intk
            INTEGER(intk), INTENT(in) :: ilevel_index, igrid, iface, &
                iworkpackage
        END SUBROUTINE boundary_callback_t
    END INTERFACE

    INTEGER(intk), PARAMETER :: workpackage_size = 2
    ! Work packages for parallel processing of boundary pressure
    ! Avoids repeated loops over all faces and checks for 'PAR' boundaries
    !   Field 1: grid index
    !   Field 2: face index
    INTEGER(intk), ALLOCATABLE :: workpackages(:, :, :)
    INTEGER(intk), ALLOCATABLE :: nworkpackages_per_level(:)
    !$omp declare target(workpackages)
...
CONTAINS
    SUBROUTINE init_bound_pressure()
        ...
        ALLOCATE(nworkpackages_per_level(nlevels), source=0_intk)

        CALL for_all_par_boundaries(boundary_counter=nworkpackages_per_level)
        ALLOCATE(workpackages(workpackage_size, &
            MAXVAL(nworkpackages_per_level), nlevels), source=-1_intk)

        CALL for_all_par_boundaries(on_boundary=add_par_boundary_workpackage)

        !$omp target enter data map(always, to: workpackages)
    ...
    SUBROUTINE add_par_boundary_workpackage(ilevel_index, igrid, iface, &
        iworkpackage)
        ! Subroutine arguments
        INTEGER(intk), INTENT(in) :: ilevel_index, igrid, iface, iworkpackage

        ! Local variables
        ! none...

        workpackages(1, iworkpackage, ilevel_index) = igrid
        workpackages(2, iworkpackage, ilevel_index) = iface
    END SUBROUTINE add_par_boundary_workpackage
    ...
    SUBROUTINE bound_pressure(ilevel, dp_f, bp_f)
        ...
        CALL roctxrangepush("bound_pressure")
        IF (use_bp) THEN
    ...
    SUBROUTINE bound_pressure_impl_bp(ilevel, dp_f, bp_f, dx_f, dy_f, dz_f, &
        ...
        INTEGER(intk) :: nworkpackages, ilevel_index, i, igrid, iface
        INTEGER(intk) :: kk, jj, ii

        CALL level_index(ilevel_index, ilevel)
        nworkpackages = nworkpackages_per_level(ilevel_index)

        !$omp target teams distribute private(i, igrid, iface, kk, jj, ii, &
        !$omp& pbuffer, bp, p, dx, dy, dz, ddx, ddy, ddz)
        DO i = 1, nworkpackages
            igrid = workpackages(1, i, ilevel_index)
            iface = workpackages(2, i, ilevel_index)
    ...
    SUBROUTINE finish_bound_pressure()
        ...
        !$omp target exit data map(always, delete: workpackages)
        DEALLOCATE(workpackages)
        DEALLOCATE(nworkpackages_per_level)
