...
CONTAINS
    SUBROUTINE bound_pressure(ilevel, dp_f, bp_f)
        ...
        IF (use_bp) THEN
            !$omp target teams distribute private(i, igrid, kk, jj, ii, iface, &
            !$omp& nbocd, ibocd, ctyp, p, pbuffer, bp, dx, dy, dz, ddx, ddy, ddz)
        ...
        ELSE
            !$omp target teams distribute private(i, igrid, kk, jj, ii, iface, &
            !$omp& nbocd, ibocd, ctyp, p, pbuffer, dx, dy, dz, ddx, ddy, ddz)
            DO i = 1, nmygridslvl(ilevel)
                igrid = mygridslvl(i, ilevel)
                CALL get_mgdims(kk, jj, ii, igrid)
