...
CONTAINS
    ...
    SUBROUTINE tstsca4(qtu_f, qtv_f, qtw_f, t_f, sca)
        ...
        DO i = 1, nmygrids
            ...
            CALL get_grid3_new(qtu, qtu_f, igrid)
            CALL get_grid3_new(qtv, qtv_f, igrid)
            CALL get_grid3_new(qtw, qtw_f, igrid)

            CALL get_grid3_new(t, t_f, igrid)
            CALL get_grid3_new(u, u_f, igrid)
            CALL get_grid3_new(v, v_f, igrid)
            CALL get_grid3_new(w, w_f, igrid)
            CALL get_grid3_new(g, g_f, igrid)
            CALL get_grid3_new(bt, bt_f, igrid)

            CALL get_grid1_new(ddx, ddx_f, igrid)
            CALL get_grid1_new(ddy, ddy_f, igrid)
            CALL get_grid1_new(ddz, ddz_f, igrid)
            CALL get_grid1_new(rdx, rdx_f, igrid)
            CALL get_grid1_new(rdy, rdy_f, igrid)
            CALL get_grid1_new(rdz, rdz_f, igrid)
    ...
    SUBROUTINE fluxbalance(qtt_f, qtu_f, qtv_f, qtw_f)
        ...
        DO i = 1, nmygrids
            ...
            CALL get_mgdims(kk, jj, ii, igrid)
            CALL get_grid3_new(qtt, qtt_f, igrid)
            CALL get_grid3_new(qtu, qtu_f, igrid)
            CALL get_grid3_new(qtv, qtv_f, igrid)
            CALL get_grid3_new(qtw, qtw_f, igrid)
            CALL get_grid1_new(rddx, rddx_f, igrid)
            CALL get_grid1_new(rddy, rddy_f, igrid)
            CALL get_grid1_new(rddz, rddz_f, igrid)
